...
def get_program_year_filters(df: pd.DataFrame):
    ...
    # --------------------------
    # PROGRAMA (con opción nulo)
    # --------------------------
    prog_series = df[prog_col].astype(str).str.strip()
    prog_series = prog_series.replace(["", "nan", "NaN", "NULL", "null"], pd.NA)

    programas_validos = (
        prog_series.dropna().unique().tolist()
    )
    programas_validos = sorted(programas_validos)

    hay_prog_nulos = prog_series.isna().any()

    opciones_prog = ["Todos los programas"] + programas_validos
    if hay_prog_nulos:
        opciones_prog.append("Sin programa / nulo")

    # --------------------------
    # AÑO DE EGRESO (con opción nulo)
    # --------------------------
    year_series = df[year_col].astype(str).str.strip()
    year_series = year_series.replace(["", "nan", "NaN", "NULL", "null"], pd.NA)

    years_encontrados = []
    for val in year_series.dropna():
        m = re.search(r"(19\d{2}|20\d{2}|2100)", val)
        if m:
            years_encontrados.append(m.group(0))

    years = sorted(set(years_encontrados), key=int)
    hay_year_nulos = year_series.isna().any()

    opciones_year = ["Todos"] + years
    if hay_year_nulos:
        opciones_year.append("Sin año de egreso / nulo")
    ...
    # Programa
    if programa_sel == "Sin programa / nulo":
        mask = mask & prog_series.isna()
    elif programa_sel != "Todos los programas":
        mask = mask & prog_series.eq(programa_sel)

    # Año
    if year_sel == "Sin año de egreso / nulo":
        mask = mask & year_series.isna()
    elif year_sel != "Todos":
        mask = mask & year_series.fillna("").str.contains(year_sel)
...
.stPlotlyChart {{
    ...
    <div class="portada-box">
      <h1>Informe de Encuesta de Egresados Posgrados</h1>
    ...
    section_titles = {
        "A": "Datos sociodemográficos y laborales",
        "B": "Satisfacción con el programa de posgrado",
        "C": "Comunicación y seguimiento",
        "D": "Producción científica y aportes sociales",
        "E": "Emprendimiento, educación continuada y vinculación con la UTP"
    }
